...
<template>
	...
	>
		...
		<div v-if="item.platform.length > 1" class="mx-7 mb-5 flex flex-row">
			...
			<div
				v-for="platform in item.platform" :key="platform" class="w-5 h-5 mr-1 block"
			...
			>
				<img
					...
					:alt="item.title" referrerpolicy="no-referrer" loading="lazy"
				>
			</div>
		</div>
		<div v-if="image && !section.noImage" class="max-h-48 relative overflow-hidden">
			<NuxtImg
				class="w-full" format="webp" :src="image" :alt="item.title" referrerpolicy="no-referrer" loading="lazy"
